...
import { postGoogleLogin, type GoogleLoginData } from '../api/google';
import { isNativeSocialAvailable } from '../lib/native-social';

/**
 * 구글 로그인 훅.
 * 구글 idToken 획득 → 백엔드(/auth/google) 전송 → 앱 토큰 저장까지 수행하고,
 * 라우팅 분기에 쓸 GoogleLoginData(isNewUser 포함)를 반환한다.
 * 사용자가 취소하면 null 반환.
 *
...
export function useGoogleLogin() {
  ...
  const signIn = async (): Promise<GoogleLoginData | null> => {
    ...
    try {
      ...
      const data = await postGoogleLogin(idToken);
